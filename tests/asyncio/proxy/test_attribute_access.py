...
async def test_attr_can_be_read_using_get_prefix(
    proxy: ActorProxy[PropertyActor],
) -> None:
    assert await proxy.get("an_attr") == "an_attr"
...
async def test_attr_can_be_set_using_set(
    ...
    assert await proxy.an_attr == "an_attr_2"  # type: ignore[attr-defined]
...
async def test_property_can_be_set(
    ...
    assert await proxy.a_rw_property == "a_rw_property_2"  # type: ignore[attr-defined]
