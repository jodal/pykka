...
class ActorForProxying(Actor):
    a_nested_object = traversable(NestedObject())
...
async def test_refs_proxy_method_returns_a_proxy(
    actor_class: type[ActorForProxying],
) -> None:
    ...
    await proxy_from_ref_proxy.stop().get()


async def test_proxy_constructor_raises_exception_if_actor_is_dead(
...
async def test_actor_ref_may_be_retrieved_from_proxy_if_actor_is_dead(
    ...
    await proxy.actor_ref.stop()

    assert not proxy.actor_ref.is_alive()


async def test_actor_proxy_does_not_expose_proxy_to_self(
    runtime: Runtime,
