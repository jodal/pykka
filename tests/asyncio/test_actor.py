...
async def test_messages_left_in_queue_after_actor_stops_receive_an_error(
    runtime: Runtime,
...
async def test_stop_requests_left_in_queue_after_actor_stops_are_handled(
    runtime: Runtime,
    actor_ref: ActorRef[AnActor],
) -> None:
    event = runtime.event_class()
...
async def test_actor_has_unique_uuid(
    actor_class: type[AnActor],
    events: Events,
) -> None:
...
async def test_on_start_is_called_before_first_message_is_processed(
    actor_ref: ActorRef[AnActor],
    events: Events,
) -> None:
...
async def test_on_start_is_called_after_the_actor_is_registered(
    actor_ref: ActorRef[AnActor],
    events: Events,
) -> None:
    ...
    await events.actor_registered_before_on_start_was_called.wait(0.1)
    assert events.actor_registered_before_on_start_was_called.is_set()


async def test_on_start_can_stop_actor_before_receive_loop_is_started(
...
async def test_on_start_failure_causes_actor_to_stop(
...
async def test_on_stop_is_called_when_actor_is_stopped(
    actor_ref: ActorRef[AnActor],
    events: Events,
) -> None:
...
async def test_on_stop_failure_causes_actor_to_stop(
...
async def test_on_failure_is_called_when_exception_cannot_be_returned(
    actor_ref: ActorRef[AnActor],
    events: Events,
) -> None:
...
async def test_on_failure_failure_causes_actor_to_stop(
...
async def test_actor_is_stopped_when_unhandled_exceptions_are_raised(
    actor_ref: ActorRef[AnActor],
    events: Events,
) -> None:
    assert not events.on_failure_was_called.is_set()
...
async def test_all_actors_are_stopped_on_base_exception(
    actor_ref: ActorRef[AnActor],
    events: Events,
) -> None:
...
async def test_actor_can_call_stop_on_self_multiple_times(
    actor_ref: ActorRef[AnActor],
) -> None:
...
async def test_actor_processes_all_messages_before_stop_on_self_stops_it(
    ...
    await actor_ref.ask({"command": "message self then stop"})

    await events.greetings_was_received.wait(5)
